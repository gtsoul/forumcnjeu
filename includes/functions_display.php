	...
	foreach ($forum_rows as $row)
	{
		...
		if ($row['parent_id'] == $root_data['forum_id'] && $row['forum_type'] == FORUM_CAT)
		{
			$template->assign_block_vars('forumrow', array(
				...
				'U_COLLAPSEFORUM'		=> append_sid("{$phpbb_root_path}index.$phpEx", 'mark=collapse&amp;f=' . $row['forum_id']),	//MOD: Collapse categories
				'FORUM_ID'				=> $row['forum_id'],
		...
		if (isset($subforums[$forum_id]))
		{
			...
			$l_subforums = (sizeof($subforums[$forum_id]) == 1) ? $user->lang['SUBFORUM'] . ':<br />' : $user->lang['SUBFORUMS'] . ':<br />';
		...
		//Début modif sous-forums en colonnes
		//$s_subforums_list = (string) implode('<br />', $s_subforums_list);
		$s_subforums_list_buffer = '<table><tr>';
		$i=1;
		foreach ($s_subforums_list as $value)
		{
			$s_subforums_list_buffer = $s_subforums_list_buffer . '<td>' . $value . '&#8193;&#8192;</td>';
			$i++;
			if ($i > 3) //Nombre de sous-forums par ligne
			{
				$s_subforums_list_buffer = $s_subforums_list_buffer . '</tr><tr>';
				$i=1;
			}
		}
		$s_subforums_list = $s_subforums_list_buffer . '</tr></table>';
		unset($s_subforums_list_buffer);
		//Fin modif sous-forums en colonnes
