<?php if (!defined('IN_PHPBB')) exit; $_forumrow_count = (isset($this->_tpldata['forumrow'])) ? sizeof($this->_tpldata['forumrow']) : 0;if ($_forumrow_count) {for ($_forumrow_i = 0; $_forumrow_i < $_forumrow_count; ++$_forumrow_i){$_forumrow_val = &$this->_tpldata['forumrow'][$_forumrow_i]; if (( $_forumrow_val['S_IS_CAT'] && ! $_forumrow_val['S_FIRST_ROW'] ) || $_forumrow_val['S_NO_CAT']) {  ?>
	...
	<?php } if ($_forumrow_val['S_IS_CAT'] || $_forumrow_val['S_FIRST_ROW'] || $_forumrow_val['S_NO_CAT']) {  ?>
		...
		<div class="forabg">
			...
			<?php if ($_forumrow_val['FORUM_IMAGE']) {  ?><div class="forum-image" style="float:left"><?php echo $_forumrow_val['FORUM_IMAGE']; ?></div><?php } ?>

			<ul class="topiclist">
				<li class="header">
					<dl class="icon", style="margin-left:32px">
						...
						<dd class="lastpost"><span><?php echo ((isset($this->_rootref['L_LAST_POST'])) ? $this->_rootref['L_LAST_POST'] : ((isset($user->lang['LAST_POST'])) ? $user->lang['LAST_POST'] : '{ LAST_POST }')); ?>

							<?php if ($this->_rootref['SCRIPT_NAME'] == ("index")) {  ?><span style="float:right"><?php if ($this->_rootref['S_USER_LOGGED_IN']) {  ?><a href="<?php echo $_forumrow_val['U_COLLAPSEFORUM']; ?>"><?php if ($_forumrow_val['S_IS_CAT_HIDDEN']) {  ?><img src="<?php echo (isset($this->_rootref['T_THEME_PATH'])) ? $this->_rootref['T_THEME_PATH'] : ''; ?>/images/collapse-active.png" alt="[+]"><?php } else { ?><img src="<?php echo (isset($this->_rootref['T_THEME_PATH'])) ? $this->_rootref['T_THEME_PATH'] : ''; ?>/images/collapse-inactive.png" alt="[&mdash;]"><?php } ?></a><?php } ?></span><?php } ?>

							</span></dd>
					</dl>
				</li>
			</ul>
			<ul class="topiclist forums" <?php if ($_forumrow_val['S_IS_CAT_HIDDEN']) {  ?> style="display:none" <?php } else { ?> style="margin-left:32px" <?php } ?>>
